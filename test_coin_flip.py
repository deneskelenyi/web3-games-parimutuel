...
BASE_URL = "http://127.0.0.1:8090/coin-flip"
...
def place_backer_bets(config):
    """Place one bet on every streak bucket so at least one always wins."""
    rpc = config["rpc_http_url"]
    address = Web3.to_checksum_address(config["contracts"]["coin-flip"])
    faucet_key = config["faucet"]["private_key"]
    abi = requests.get(f"http://127.0.0.1:{config['port']}/api/abi/coin-flip").json()
    ...
    for bet_type in range(7):
        ...
        if receipt.status != 1:
            raise RuntimeError(f"Backer bet failed for type={bet_type}")
        print(f"Backer bet placed: round={current_round} type={bet_type}")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.screenshot(path=str(ROOT / "coin_flip_test_1_loaded.png"))
        page.click("#demoWalletBtn")
        page.wait_for_timeout(3000)

        page.screenshot(path=str(ROOT / "coin_flip_test_2_connected.png"))
        ...
        # Pick the 6+ HEADS bucket (bet type 6)
        page.click('.bet-type-btn[data-type="6"]')
        ...
        page.screenshot(path=str(ROOT / "coin_flip_test_3_bet_placed.png"))
        ...
        settled = page.wait_for_function(
            ...
            () => {
                ...
                const hasOutcome = txt && (txt.includes('HEADS') || txt.includes('STREAK'));
        ...
        assert "HEADS" in outcome, f"Unexpected outcome: {outcome}"

        page.screenshot(path=str(ROOT / "coin_flip_test_4_settled.png"))
        ...
        page.screenshot(path=str(ROOT / "coin_flip_test_5_final.png"))

        print("Coin Flip Streak test passed")
