...
BASE_URL = "http://127.0.0.1:8090/roulette"
ROOT = Path(__file__).resolve().parent

BACKER_KEY = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d"
...
def place_backer_bets(config):
    """Place bets that guarantee at least one winner for any roulette outcome."""
    rpc = config["rpc_http_url"]
    address = Web3.to_checksum_address(config["contracts"]["roulette"])
    faucet_key = config["faucet"]["private_key"]
    abi = requests.get(f"http://127.0.0.1:{config['port']}/api/abi/roulette").json()

    w3 = Web3(Web3.HTTPProvider(rpc))
    contract = w3.eth.contract(address=address, abi=abi)
    faucet = Account.from_key(faucet_key)
    backer = Account.from_key(BACKER_KEY)

    bal = w3.eth.get_balance(backer.address)
    ...
    for bet_type, bet_value in bets:
        tx = contract.functions.placeBet(bet_type, bet_value).build_transaction({
            "from": backer.address,
        ...
        signed = w3.eth.account.sign_transaction(tx, BACKER_KEY)
        raw = getattr(signed, "raw_transaction", getattr(signed, "rawTransaction", None))
        tx_hash = w3.eth.send_raw_transaction(raw)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=30)
        if receipt.status != 1:
            raise RuntimeError(f"Backer bet failed for type={bet_type} value={bet_value}")
        print(f"Backer bet placed: round={current_round} type={bet_type} value={bet_value}")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.screenshot(path=str(ROOT / "roulette_test_1_loaded.png"))
        page.click("#demoWalletBtn")
        page.wait_for_timeout(5000)

        page.screenshot(path=str(ROOT / "roulette_test_2_connected.png"))
        ...
        # Set amount first, then pick RED (bet slip requires an amount)
        page.fill("#betAmount", "0.01")
        page.wait_for_timeout(200)
        page.click('.bet-type-btn[data-type="0"]')
        ...
        page.screenshot(path=str(ROOT / "roulette_test_3_bet_placed.png"))
        ...
        settled = page.wait_for_function(
            ...
            () => {
                ...
                const hasOutcome = txt && txt.includes('—');
        ...
        assert any(s in outcome for s in ["RED", "BLACK", "GREEN"]), f"Unexpected outcome: {outcome}"

        page.screenshot(path=str(ROOT / "roulette_test_4_settled.png"))
        ...
        page.screenshot(path=str(ROOT / "roulette_test_5_final.png"))
