...
BASE_URL = "http://127.0.0.1:8090/plinko"
...
def place_backer_bets(config, demo_address):
    ...
    address = Web3.to_checksum_address(config["contracts"]["plinko"])
    faucet_key = config["faucet"]["private_key"]
    abi = requests.get(f"http://127.0.0.1:{config['port']}/api/abi/plinko").json()
    ...
    for key, zone in zip(BACKER_KEYS, BACKER_ZONES):
        acct = Account.from_key(key)
        tx = contract.functions.placeBet(0, zone).build_transaction({
            "from": acct.address,
        ...
        print(f"Backer bet placed: round={current_round} zone={zone} from {acct.address}")
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Pick zone 6
        page.click('.zone-btn[data-value="6"]')
        ...
        settled = page.wait_for_function(
            ...
            () => {
                ...
                const hasOutcome = txt && txt.includes('Landed');
        ...
        assert "zone" in outcome, f"Unexpected outcome: {outcome}"
        ...
        page.screenshot(path=str(ROOT / "plinko_test_settled.png"))

        print("Plinko test passed")
